...
```bash
sudo dpkg -i KarinCore_1.2.8_amd64.deb
...
* **Fixed:** the app was redundantly re-downloading and reinstalling Xray on every launch when it was already available as a system package (e.g. on Arch/AUR/SteamOS) at a different path — this also caused a spurious sudo password prompt on every startup.
